---
title: Verwenden der AEM-Desktop-App, Version 1.x
description: Erfahren Sie, wie Sie die Adobe Experience Manager-Desktop-App, Version 1.x verwenden und Ihre Arbeit mit Assets auf dem Desktop optimieren können.
...
# Verwenden der AEM-Desktop-App, v1.x {#use-aem-desktop-app-v1x}

Mithilfe der App haben Sie über Ihren lokalen Desktop problemlosen Zugriff auf die Assets in AEM und die Assets können in beliebigen Desktop-Applikationen verwendet werden. Assets können in Mac Finder oder Windows Explorer leicht angezeigt, in Desktop-Applikationen geöffnet und lokal geändert werden. Die Änderungen werden dann wieder unter einer neuen, im Repository erstellten Version in AEM gespeichert.
...
Zu den Hauptaufgaben, die Sie mit der AEM-Desktop-App v1 ausführen, gehören:
...
>[!NOTE]
>Die AEM-Desktop-App wurde in der AEM-Version 6.1 unter der Bezeichnung AEM Assets Companion App eingeführt.

## Touchpoints der AEM-Desktop-App im kreativen Workflow {#aem-desktop-app-touch-points-in-the-creative-workflow}

Die AEM Desktop-App integriert sich zusammen mit AEM Assets in Ihren kreativen Workflow und bietet die folgenden Touchpoints.
...
## Installieren der AEM-Desktop-App und Verbinden mit dem AEM-Server {#installandconnect}
...
Die AEM-Desktop-App verwendet eine HTTPS-Verbindung, um eine Verbindung zum AEM-Server herzustellen, damit Ihre Assets sicher übertragen werden.
...
### Installieren der App {#installapp}

Stellen Sie für die Verwendung der AEM-Desktop-App sicher, dass Ihre AEM-Server-Version von der AEM-Desktop-App unterstützt wird. Laden Sie die entsprechende Installationsdatei (binär) für Ihr Betriebssystem (Mac oder Windows) herunter und installieren Sie die App.

Je nach Netzwerk- und Systemvoreinstellungen kann eine detaillierte Konfiguration erforderlich sein. Weitere Einzelheiten dazu finden Sie unter [Installieren und Konfigurieren der AEM-Desktop-App](install-configure-app-v1.md).

1. Öffnen Sie die [Download-Seite der AEM-Desktop-App](https://helpx.adobe.com/experience-manager/kb/download-companion-app.html) und laden Sie die entsprechende Binärdatei für Ihr Betriebssystem herunter.
...
**Asset-Status einsehen** Wenn geänderte Assets hochgeladen oder dem AEM Assets-Repository neue Assets hinzugefügt werden, lädt die App die Assets im Hintergrund hoch. Der Upload im Hintergrund ermöglicht die reibungslose Ausführung von Vorgängen, ohne dass der Abschluss des Uploads abgewartet werden muss. Dies ist insbesondere bei umfangreichen Assets hilfreich. Sie können die Änderungen lokal speichern und müssen sich nicht mehr darum kümmern. Je nach verfügbarer Bandbreite nimmt das Senden dieser Assets an den Server durch die Applikation etwas Zeit in Anspruch. Sie können den Status des Uploads sowie weitere grundlegende Informationen überprüfen.

**Options (Optionen)** Klicken/tippen Sie in der Ablage der AEM-Desktop-App auf „Options“ (Optionen), um auf Einstellungen zuzugreifen, mit denen Sie festlegen, ob die App beim Systemstart gestartet wird und ob beim App-Start eine Verbindung zum AEM-Server hergestellt wird. Außerdem können Sie hier den Buchstaben des lokalen Laufwerks ändern, auf dem AEM Assets nach der Bereitstellung verfügbar ist.
...
>[!NOTE]
>
>Wenn Sie die Proxy-Einstellungen ändern, müssen Sie die App neu starten, damit die Änderungen übernommen werden. Andernfalls verwendet die App weiterhin den zuvor konfigurierten Proxyserver.

1. Starten Sie die AEM-Desktop-App. Wenn Sie Ihre AEM-Instanz der App zuordnen möchten, geben Sie Ihren AEM-Server im folgenden Format an `https://[aem-server-url]:[port]`.
...
>[!NOTE]
>Unter Windows verhindert die [Windows 7-Standardeinstellung](https://support.microsoft.com/en-us/kb/2668751), dass die AEM-Desktop-App Dateien verarbeitet, die größer als 50 MB sind.
...
Wählen Sie auf einem Mac im Kontextmenü die Option „Öffnen“ aus, um ein Asset über die AEM-Desktop-App zu öffnen.

Wählen Sie bei Adobe InDesign-Dateien (INDD) im Kontextmenü die Option **[!UICONTROL Open]** aus. Wenn Sie auf diese Option klicken, lädt die App die verknüpften Assets in Ihr lokales Dateisystem herunter und öffnet anschließend die INDD-Datei in Adobe InDesign. Mithilfe dieser Methode wird sichergestellt, dass die erforderlichen Assets beim Bearbeiten einer INDD-Datei lokal verfügbar sind.
...
| ![Windows-Standardapplikationssymbol](assets/win_default.png) | Die App ist mit dem Server verbunden und alle Assets sind synchronisiert. |
|------|-----------------------------------------------------------------------------------------------------------------------------------------------------------|
...
Mithilfe der App können Sie Dateien auschecken, damit andere Benutzer keine Änderungen an den Dateien vornehmen können, an denen Sie gerade arbeiten.
...
1. Klicken/tippen Sie im Dialogfeld „Asset Info“ (Asset-Informationen) auf das Symbol „Auschecken“, um das Asset auszuchecken. Das Checkout-Symbol wechselt zum Checkin-Symbol, nachdem Sie darauf geklickt/getippt haben.

   ![Zum Asset navigieren um auszuchecken](assets/chlimage_1-6.png)

1. Um das Asset so einzuchecken, dass es für andere Benutzer verfügbar ist, klicken Sie im Dialogfeld "Asset-Informationen"auf das Symbol zum Einchecken/Tippen.
...
1. Klicken/tippen Sie im Dialogfeld „Asset Info“ (Asset-Informationen) auf das Symbol „Checkout“ (Auschecken). Das Symbol Kasse wechselt zum Symbol zum Einchecken.
...
Wenn Sie mit dem Bearbeiten der Assets fertig sind, speichern Sie sie in Ihrer Desktop-Applikation. Wählen Sie im Kontextmenü Mehr Assets Info und klicken Sie auf Checkin/tippen Sie auf.
...
Alternativ können Sie ein Asset über die AEM-Web-Benutzeroberfläche einchecken. Klicken/tippen Sie auf die ausgecheckten Assets oder wählen Sie sie aus. From the toolbar, click/tap the check in icon ![check-in icon](assets/aemassets_icon_checkin.png).
